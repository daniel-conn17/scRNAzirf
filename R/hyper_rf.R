...
#'                     effect sizes.
#' @param pca_genes genes used for pca
#' @param inputTF input transcription factors
#' @return A list.
#' @export
hyper_rf_fit <- function(x, y, mtry,
  ...
  xperm <- x[perm_ind, ]
  y <- y[, 1]; yperm <- y[perm_ind]
  x <- x[!perm_ind, ]; y <- y[!perm_ind]
  x_ind <- 1:dim(x)[2]
  ...
  lm_dat <- data.frame(x, y)
  names(lm_dat)[dim(lm_dat)[2]] <- yname

  #create formula
  mod_formula <- X_to_form(yname, x)
  ...
  lm_mod <- stats::lm(mod_formula, data = lm_dat)
  ...
  x <- filter_TFs(x)
  newx_linear_cut <- as.matrix(cbind(rep(1, dim(newx)[1]), newx))%*%lm_coef
  ...
  nnew <- dim(newx)[1]
      rf_fit <- ranger::ranger(y ~ ., data=ranger_dat, mtry=mtry,
                               ...
                               num.trees = ntree)
  rf_preds <- stats::predict(rf_fit, data=x)$predictions
  new_rf_preds <- stats::predict(rf_fit, data=newx)$predictions
  new_fit <- data.frame(lambda_preds=new_rf_preds)
  data_fit <- data.frame(lambda_preds=rf_preds)
  ...
  xperm_linear_cut <- df_mult(xperm, lm_coef)
  ...
  for(l in 1:length(inputTF)){
    ...
    perturbed_covariate$linear_cut <- df_mult(perturbed_covariate[, names(lm_coef[-1])],
                                              lm_coef)
